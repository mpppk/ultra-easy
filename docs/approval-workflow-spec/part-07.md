...
type ObjectExpression \=  
  | { type: "reference"; objectType: AuthorizationObjectType; id: ValueExpression }  
  | { type: "literal"; object: AuthorizationObjectRef };

Approval Step key、relation名、authorization object type/referenceも意味の異なる識別子としてbranded typeを使う。brandはJSON ASTへ追加fieldを生成せず、TypeScript上の取り違え防止だけを担う。
