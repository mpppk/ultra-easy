...
bun workspace (`workspaces: ["apps/*", "packages/*"]`), following the layout Vite+ scaffolds in `node_modules/vite-plus/templates/monorepo`. **The workspace root holds no implementation** — apps go in `apps/`, libraries in `packages/`.
...
tests/                    milestone-spanning acceptance suite (see docs/implementation-plan.md)
```
...
- `vp check` and `vp test` do run at the root and sweep every package — that is what CI uses.
...
- Validate from the root with `vp check` and `vp test`.
