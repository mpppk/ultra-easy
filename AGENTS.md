...
apps/knowledge/           @app/knowledge — Knowledge Workspace example app (#167, see apps/knowledge/README.md)
packages/approval-core/   @app/approval-core — approval workflow domain core
packages/knowledge-core/  @app/knowledge-core — Knowledge domain (pages, revisions, snapshots, projections)
packages/knowledge-d1/    @app/knowledge-d1 — Knowledge D1 migrations + repositories (FTS5)
...
- A single root `tsconfig.json` covers every package, so the type-aware lint in `vp check` sees the whole workspace as one TS project. Exception: `apps/knowledge` has its own `tsconfig.json` (excluded from the root one) because a second TanStack Start app's router module augmentation would clash with `apps/web`'s; `vp check` still lints and type-checks it.
- `apps/knowledge` is an example _consumer_ of ultra-easy: it must not import `@app/approval-*` packages. It talks to ultra-easy only through `src/ultra-easy/client.ts` (today backed by the in-app mock in `src/ultra-easy/mock`, until the Workflow Engine public API exists).
